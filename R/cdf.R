...
#
   if (ld$is.discrete) {
    ...
    return(cumsum(ld$dpdf[1:x])[x])
  }
  leftLimit <- ld$range[1] - 5*ld$bw
  rightLimit <- ld$range[2] + 5*ld$bw
  integrate(ld$lossdistr, lower = max(1,leftLimit), upper = min(x,rightLimit))$value * ld$normalizationFactor
}
